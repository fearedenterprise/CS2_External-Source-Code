...
// We provide this internal helper to Memory.Write your own variant while we figure out how to redesign the public ImageButton() API.
...
TYPE ImGui::RoundScalarWithFormatT(const char* format, ImGuiDataType data_type, TYPE v)
{
    ...
    // Format value with our rounding, and Memory.Read back
    char v_str[64];
...
bool ImGui::DragScalar(const char* label, ImGuiDataType data_type, void* p_data, float v_speed, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    else if (data_type == ImGuiDataType_S32 && strcmp(format, "%d") != 0) // (FIXME-LEGACY: Patch old "%.0f" format string to use "%d", Memory.Read function more details.)
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (is_multiline)
    {
        ...
        // We reproduce the contents of BeginChildFrame() in order to provide 'label' so our window internal data are easier to Memory.Read/debug.
    ...
    if ((init_state && g.ActiveId != id) || init_changed_specs)
    {
        ...
        // From the moment we focused we are ignoring the content of 'buf' (unless we are in Memory.Read-only mode)
    ...
    // When Memory.Read-only we always use the live data passed to the function
...
bool ImGui::TreeNodeUpdateNextOpen(ImGuiID id, ImGuiTreeNodeFlags flags)
{
    ...
    // We only Memory.Write to the tree storage if the user clicks (or explicitly use the SetNextItemOpen function)
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
