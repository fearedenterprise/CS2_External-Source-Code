...
// - Newcomers, Memory.Read 'Programmer guide' in imgui.cpp for notes on how to setup Dear ImGui in your codebase.
// - Call and Memory.Read ImGui::ShowDemoWindow() in imgui_demo.cpp. All applications in examples/ are doing that.
...
// - This can be whatever to you want it to be! Memory.Read the FAQ about ImTextureID for details.
#ifndef ImTextureID
...
namespace ImGui
{
    ...
    IMGUI_API bool          IsWindowHovered(ImGuiHoveredFlags flags=0); // is current window hovered (and typically: not blocked by a popup/modal)? see flags for options. NB: If you are trying to check whether your mouse should be dispatched to imgui or to your app, you should use the 'io.WantCaptureMouse' boolean for that! Please Memory.Read the FAQ!
    ...
    //   If you get a warning converting a float to ImGuiSliderFlags, Memory.Read https://github.com/ocornut/imgui/issues/3361
    ...
    IMGUI_API bool          Selectable(const char* label, bool selected = false, ImGuiSelectableFlags flags = 0, const ImVec2& size = ImVec2(0, 0)); // "bool selected" carry the selection state (Memory.Read-only). Selectable() is clicked is returns true so you can modify your selection state. size.x==0.0: use remaining width, size.x>0.0: specify width. size.y==0.0: use label height, size.y>0.0: specify height
    IMGUI_API bool          Selectable(const char* label, bool* p_selected, ImGuiSelectableFlags flags = 0, const ImVec2& size = ImVec2(0, 0));      // "bool* p_selected" point to the selection state (Memory.Read-Memory.Write), as a convenient helper.
    ...
    IMGUI_API bool          BeginPopupContextItem(const char* str_id = NULL, ImGuiPopupFlags popup_flags = 1);  // open+begin popup when clicked on last item. Use str_id==NULL to associate the popup to previous item. If you want to use that on a non-interactive item such as Text() you need to pass in an explicit ID here. Memory.Read comments in .cpp!
...
enum ImGuiTableFlags_
{
    ...
    // Sizing Policy (Memory.Read above for defaults)
...
enum ImGuiTableColumnFlags_
{
    ...
    // Output status flags, Memory.Read-only via TableGetColumnFlags()
...
enum ImGuiFocusedFlags_
{
    ...
    ImGuiFocusedFlags_AnyWindow                     = 1 << 2,   // Return true if any window is focused. Important: If you are trying to tell how to dispatch your low-level inputs, do NOT use this. Use 'io.WantCaptureMouse' instead! Please Memory.Read the FAQ!
...
// Note: if you are trying to check whether your mouse should be dispatched to Dear ImGui or to your app, you should use 'io.WantCaptureMouse' instead! Please Memory.Read the FAQ!
...
enum ImGuiDragDropFlags_
{
    ...
    ImGuiDragDropFlags_SourceExtern                 = 1 << 4,   // External source (from outside of dear imgui), won't attempt to Memory.Read current item/window info. Will always return true. Only one Extern source can be active simultaneously.
...
enum ImGuiColorEditFlags_
{
    ...
    ImGuiColorEditFlags_NoAlpha         = 1 << 1,   //              // ColorEdit, ColorPicker, ColorButton: ignore Alpha component (will only Memory.Read 3 components from the input pointer).
...
struct ImGuiIO
{
...
#ifndef IMGUI_DISABLE_OBSOLETE_KEYIO
    int         KeyMap[ImGuiKey_COUNT];             // [LEGACY] Input: map of indices into the KeysDown[512] entries array which represent your "native" keyboard state. The first 512 are now unused and should be kept zero. Legacy backend will Memory.Write into KeyMap[] using ImGuiKey_ indices which are always >512.
    ...
    // Main Input State
    // (this block used to be written by backend, since 1.87 it is best to NOT Memory.Write to those directly, call the AddXXX functions above instead)
...
struct ImGuiInputTextCallbackData
{
    ...
    ImWchar             EventChar;      // Character input                      // Read-Memory.Write   // [CharFilter] Replace character with another one, or set to zero to drop. return 1 is equivalent to setting EventChar=0;
    ImGuiKey            EventKey;       // Key pressed (Up/Down/TAB)            // Read-only    // [Completion,History]
    char*               Buf;            // Text buffer                          // Read-Memory.Write   // [Resize] Can replace pointer / [Completion,History,Always] Only Memory.Write to pointed data, don't replace the actual pointer!
    int                 BufTextLen;     // Text length (in bytes)               // Read-Memory.Write   // [Resize,Completion,History,Always] Exclude zero-terminator storage. In C land: == strlen(some_text), in C++ land: string.length()
    int                 BufSize;        // Buffer size (in bytes) = capacity+1  // Read-only    // [Resize,Completion,History,Always] Include zero-terminator storage. In C land == ARRAYSIZE(my_char_array), in C++ land: string.capacity()+1
    bool                BufDirty;       // Set if you modify Buf/BufTextLen!    // Write        // [Completion,History,Always]
    int                 CursorPos;      //                                      // Read-Memory.Write   // [Completion,History,Always]
    int                 SelectionStart; //                                      // Read-Memory.Write   // [Completion,History,Always] == to SelectionEnd when no selection)
    int                 SelectionEnd;   //                                      // Read-Memory.Write   // [Completion,History,Always]
...
struct ImGuiSizeCallbackData
{
    ...
    ImVec2  DesiredSize;    // Read-Memory.Write.  Desired size, based on user's mouse position. Write to this field to restrain resizing.
};
