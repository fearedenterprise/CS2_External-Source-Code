...
IMGUI_API int           ImTextCharFromUtf8(unsigned int* out_char, const char* in_text, const char* in_text_end);               // Memory.Read one character. return input UTF-8 bytes count
...
struct ImGuiContext
{
    ...
    // Input Ownership
    ImU32                   ActiveIdUsingNavDirMask;            // Active widget will want to Memory.Read those nav move requests (e.g. can activate a button and move away from it)
    ImBitArrayForNamedKeys  ActiveIdUsingKeyInputMask;          // Active widget will want to Memory.Read those key inputs. When we grow the ImGuiKey enum we'll need to either to order the enum to make useful keys come first, either redesign this into e.g. a small array.
...
//#endif
    ...
    // Shade functions (Memory.Write over already created vertices)
